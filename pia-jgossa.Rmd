...
D'après le site du Ministère, « _le Programme d’investissements d’avenir (PIA), 
piloté par le Secrétariat général pour l'investissement (SGPI), a été mis en 
place par l’État pour financer des investissements innovants et prometteurs 
sur le territoire, afin de permettre à la France d’augmenter son potentiel de 
croissance et d’emplois_ ». Dans l'enseignement supérieur et la recherche (ESR), 
l'opérateur du PIA est l'Agence Nationale de la Recherche (ANR), qui a été chargée
...
dans le périmètre du MESRI, dont la variable `ANR PIA` qui indique le montant de 
...
```{r total.pia, fig.cap = "Répartition des montants du PIA distribués par l'ANR aux établissements de l'ESR"}
anr %>%
  ...
  scale_fill_manual(values=étiquettes.pal) +
  theme_cpesr_cap()
```
...
que ce programme relève du gouvernement à distance [@gally_entre_2018], et qu'il 
...
Cette question sera abordée sous l'angle des données ouvertes, qui permettent de 
visualiser la distribution des moyens entre les établissements de l'ESR. 
L'interprétation des données ouvertes nécessite de nombreuses prudences, 
qui seront explicitées. Notamment, ces données sont aussi puissantes pour 
...
Nous allons croiser quatre jeux de données ouvertes :
...
(Figure 2). Cependant, ce constat est relativisé lorsqu'on s'intéresse également
aux autres financement (autres ressources propres `Autres RP` et subventions pour charge
de service public `SCSP`), et au total appelé produit de fonctionnement encaissable 
`PFE`, en face desquels le PIA semble moins important (Figure 3). 
...
```{r vue.pia, fig.cap = "Répartition des montants du PIA entre les universités"}
anr.pivot.2 %>% 
...
```{r vue.fin, fig.cap = "Répartition des moyens entre les universités"}
anr.pivot.2 %>% 
  filter(!etablissement %in% etab.filter) %>%
  filter(exercice == exercice.ref) %>%
  filter(groupe == "université") %>% 
...
```{r vue.pfe.tit, fig.cap = "Répartition des Produits de fonctionnement encaissables entre les universités, par E-EC titulaire"}
anr.pivot.2 %>% 
  filter(!etablissement %in% etab.filter) %>%
  filter(exercice == exercice.ref) %>%
  filter(groupe == "université") %>% 
...
```{r scsp.vs.PIA, fig.cap="Evolution des subventions pour charge de service public (SCSP) entre 2013 et 2019 selon les montants de PIA et les labellisations IDEx et ISITE"}
df <- anr %>% 
  filter(!etablissement %in% etab.filter) %>%
...
cowplot::plot_grid(ncol=2, rel_widths = c(2.5,1), align = "h", axis = "tb",
  ...
  ggplot(df, aes(x=ANR.PIA,y=SCSP.diff,color=étiquettes)) +
    ...
    scale_color_manual(values=étiquettes.pal) +
    theme_cpesr_cap() +
    theme(
...
```{r tit.vs.PIA, fig.cap="Evolution du nombre d'E-EC titulaires entre 2013 et 2019 selon les montants de PIA et les labellisations IDEx et ISITE"}
df <- anr %>% 
  filter(!etablissement %in% etab.filter) %>%
  filter(!etablissement %in% c("Université de Versailles Saint-Quentin-en-Yvelines","Université d'Artois")) %>%
  filter(groupe == "université") %>% 
