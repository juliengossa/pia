...
anr %>%
  ...
  mutate(Valeur = Valeur / étudiants) %>% 
  group_by(exercice,Indicateur) %>%
  summarise(EcartType = sd(Valeur, na.rm = TRUE)) %>%
  group_by(Indicateur) %>%
  mutate(EcartType = EcartType / first(EcartType) * 100) %>%
  ggplot(aes(x=exercice,y=EcartType,color=Indicateur)) +
